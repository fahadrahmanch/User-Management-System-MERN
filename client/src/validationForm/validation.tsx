...
export const validationForupdatePage=(values:any)=>{
const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

let errorUpdate:any={}
if (!values?.name || !values.name.trim()) {
  errorUpdate.name = 'Name is required';
} else if (values.name.trim().length < 3) {
  errorUpdate.name = 'Name must be at least 3 characters';
}
if (!values?.email || values.email.trim() === '') {
  errorUpdate.email = 'Email is required';
} else if (!emailRegex.test(values?.email)) {
  errorUpdate.email = 'Please enter a valid email address';
}
return errorUpdate
}

export const validationForAddUser=(values:any)=>{
  let error:any={}
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!values?.name || !values.name.trim()) {
      error.name = 'Name is required';
